...
void convert_to_json(std::ifstream& in, std::ostream& out, program_options const& opts)
{
  ...
  while (!in.eof()) {
    ...
    auto records = validate_fb<spark_rapids_jni::profiler::ActivityRecords>(*fb_ptr, "ActivityRecords");
    std::string json;
    char const* err = flatbuffers::GenText(parser, fb_ptr->data() + sizeof(flatbuffers::uoffset_t), &json);
    if (err != nullptr) {
...
void convert_to_nvtxw(std::ifstream& in, nvtxwInterfaceCore_t *&nvtxwInterface, nvtxwStreamHandle_t& stream, program_options const& opts)
{
      ...
      for (int i = 0; i < marker->size(); ++i) {
        ...
        if (object_id != nullptr) {
          ...
          if (m->flags() & spark_rapids_jni::profiler::MarkerFlags_Start) {
            ...
            if (it != marker_start_map.end()) {
              auto const& ms = it->second;
              NvidiaNvtxw::nvtxEvent event;
              event.time_start = ms.timestamp;
              event.time_stop = m->timestamp();
              event.name = ms.name.c_str();
              event.domain = ms.domain.c_str();
              event.process_id = ms.process_id;
              event.thread_id = ms.thread_id & 0xffffff;
              event.color = ms.color;
              nvtxPayloadData_t payloadData[] = {
                  {NvidiaNvtxw::PayloadSchemaId::nameId, strlen(event.name)+1, event.name},
                  {NvidiaNvtxw::PayloadSchemaId::nameId, strlen(event.domain)+1, event.domain},
                  {NvidiaNvtxw::PayloadSchemaId::nvtxId, sizeof(event), &event},
              };
              result = nvtxwInterface->EventWrite(stream, payloadData, std::extent<decltype(payloadData)>::value);
              if (result != NVTXW3_RESULT_SUCCESS)
              {
    ...
    if (overhead != nullptr) {
      ...
      for (int i = 0; i < overhead->size(); ++i) {
        ...
        if (object_id != nullptr) {
          ...
          event.process_id = object_id->process_id();
  ...
  try {
    ...
    if (opts.json) {
      ...
      if (opts.output_path) {
        ...
        if (errorCode == 0) {
          convert_to_nvtxw(in, nvtxwInterface, stream, opts);
